...
func TestEmbeddedCoreArtifactHasPinnedProvenance(t *testing.T) {
	...
	if got := strings.TrimSpace(string(commit)); got != "47fec2008f60ae643b3bfba6ed39faa7d0b68ce5" {
...
func TestEmbeddedCoreVersion(t *testing.T) {
	...
	if !envelope.OK || envelope.Value.SchemaVersion != 1 || envelope.Value.CoreVersion != "0.16.0" {
