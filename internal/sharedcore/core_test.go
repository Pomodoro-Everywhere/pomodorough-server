...
func TestEmbeddedCoreArtifactHasPinnedProvenance(t *testing.T) {
	...
	if got := strings.TrimSpace(string(commit)); got != "7cdc404cb5d0ae7aed0c675dfc9e1c0faf8f39af" {
...
func TestEmbeddedCoreVersion(t *testing.T) {
	...
	if !envelope.OK || envelope.Value.SchemaVersion != 1 || envelope.Value.CoreVersion != "0.30.0" {
