...
Readiness snapshot refreshed: 2026-09-13 — suite published at `0.36.0` (tag `v0.36.0` at `6f81e46`, Core `10bbfea…`/`02443da7…`); `0.37.0` target.
...
| --- | --- | --- |
| Apple iOS/macOS | Unsigned `0.36.0` published and independently verified; `0.37.0` target | Developer signing, TestFlight, Developer ID signing, and notarization are excluded by request |
| Android | API-36-targeting unsigned `0.36.0` published and independently verified; `0.37.0` target | Production-keystore signing is excluded by request |
| Desktop | Unsigned `0.36.0` published and independently verified; `0.37.0` target | Windows Authenticode signing is excluded by request |
...
| Go service | `0.36.0` published and independently verified with archives, SBOM, checksums, and attestations (tag `v0.36.0` at `6f81e46`, Core `10bbfea…`/`02443da7…`) | Production SSH deployment and an authentic encrypted-backup restore remain externally blocked |
...
| P1.9 | Complete for supported unsigned scope | All four `0.36.0` releases passed clean builds, exact-set/checksum/attestation checks, supported clean-install/runtime smoke, publication, and independent fresh-download verification (server `6f81e46`, tag `v0.36.0` pushed, release workflow success); `0.37.0` target. |
