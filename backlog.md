...
Readiness snapshot refreshed: 2026-09-08 — suite published at `0.26.0` (tag `v0.26.0` at `ae377d6`, Core `b25e0d8…`/`150d5aa6…`); `0.27.0` target.
...
| --- | --- | --- |
| Apple iOS/macOS | Unsigned `0.26.0` published and independently verified; `0.27.0` target | Developer signing, TestFlight, Developer ID signing, and notarization are excluded by request |
| Android | API-36-targeting unsigned `0.26.0` published and independently verified; `0.27.0` target | Production-keystore signing is excluded by request |
| Desktop | Unsigned `0.26.0` published and independently verified; `0.27.0` target | Windows Authenticode signing is excluded by request |
...
| Go service | `0.26.0` published and independently verified with archives, SBOM, checksums, and attestations (tag `v0.26.0` at `ae377d6`, Core `b25e0d8…`/`150d5aa6…`) | Production SSH deployment and an authentic encrypted-backup restore remain externally blocked |
...
| P1.9 | Complete for supported unsigned scope | All four `0.26.0` releases passed clean builds, exact-set/checksum/attestation checks, supported clean-install/runtime smoke, publication, and independent fresh-download verification (server `ae377d6`, tag `v0.26.0` pushed, release workflow success); `0.27.0` target. |
