...
Readiness snapshot refreshed: 2026-09-13 — suite published at `0.35.0` (tag `v0.35.0` at `43ddd18`, Core `6b273da…`/`4338e8db…`); `0.36.0` target.
...
| --- | --- | --- |
| Apple iOS/macOS | Unsigned `0.35.0` published and independently verified; `0.36.0` target | Developer signing, TestFlight, Developer ID signing, and notarization are excluded by request |
| Android | API-36-targeting unsigned `0.35.0` published and independently verified; `0.36.0` target | Production-keystore signing is excluded by request |
| Desktop | Unsigned `0.35.0` published and independently verified; `0.36.0` target | Windows Authenticode signing is excluded by request |
...
| Go service | `0.35.0` published and independently verified with archives, SBOM, checksums, and attestations (tag `v0.35.0` at `43ddd18`, Core `6b273da…`/`4338e8db…`) | Production SSH deployment and an authentic encrypted-backup restore remain externally blocked |
...
| P1.9 | Complete for supported unsigned scope | All four `0.35.0` releases passed clean builds, exact-set/checksum/attestation checks, supported clean-install/runtime smoke, publication, and independent fresh-download verification (server `43ddd18`, tag `v0.35.0` pushed, release workflow success); `0.36.0` target. |
