...
Readiness snapshot refreshed: 2026-09-06 — suite published at `0.12.0`; `0.13.0` tag failed (CI run `34040281802`: Core pin `15cdd929…` is not a Core ref, one-char typo); `0.14.0` targets Core `20f9696b…` (Go tag `v0.14.0` pending).
...
| Go service | `0.12.0` published and independently verified with archives, SBOM, checksums, and attestations; `0.13.0` tag failed (CI run `34040281802`, nonexistent Core pin), `0.14.0` target repins Core `20f9696b…` | Production SSH deployment and an authentic encrypted-backup restore remain externally blocked |
