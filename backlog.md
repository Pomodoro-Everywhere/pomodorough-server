...
Readiness snapshot refreshed: 2026-09-12 — suite published at `0.32.0` (tag `v0.32.0` at `45f2188`, Core `1d30e00…`/`ac96b058…`); `0.33.0` target.
...
| --- | --- | --- |
| Apple iOS/macOS | Unsigned `0.32.0` published and independently verified; `0.33.0` target | Developer signing, TestFlight, Developer ID signing, and notarization are excluded by request |
| Android | API-36-targeting unsigned `0.32.0` published and independently verified; `0.33.0` target | Production-keystore signing is excluded by request |
| Desktop | Unsigned `0.32.0` published and independently verified; `0.33.0` target | Windows Authenticode signing is excluded by request |
...
| Go service | `0.32.0` published and independently verified with archives, SBOM, checksums, and attestations (tag `v0.32.0` at `45f2188`, Core `1d30e00…`/`ac96b058…`) | Production SSH deployment and an authentic encrypted-backup restore remain externally blocked |
...
| P1.9 | Complete for supported unsigned scope | All four `0.32.0` releases passed clean builds, exact-set/checksum/attestation checks, supported clean-install/runtime smoke, publication, and independent fresh-download verification (server `45f2188`, tag `v0.32.0` pushed, release workflow success); `0.33.0` target. |
