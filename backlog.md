...
Readiness snapshot refreshed: 2026-09-08 — suite published at `0.20.0` (tag `v0.20.0` at `aa6bf72`, Core `9a77f1d…`); `0.21.0` target.
...
| --- | --- | --- |
| Apple iOS/macOS | Unsigned `0.20.0` published and independently verified; `0.21.0` target | Developer signing, TestFlight, Developer ID signing, and notarization are excluded by request |
| Android | API-36-targeting unsigned `0.20.0` published and independently verified; `0.21.0` target | Production-keystore signing is excluded by request |
| Desktop | Unsigned `0.20.0` published and independently verified; `0.21.0` target | Windows Authenticode signing is excluded by request |
...
| Go service | `0.20.0` published and independently verified with archives, SBOM, checksums, and attestations (tag `v0.20.0` at `aa6bf72`, Core `9a77f1d…`) | Production SSH deployment and an authentic encrypted-backup restore remain externally blocked |
...
| P1.9 | Complete for supported unsigned scope | All four `0.20.0` releases passed clean builds, exact-set/checksum/attestation checks, supported clean-install/runtime smoke, publication, and independent fresh-download verification (server `aa6bf72`, tag `v0.20.0` pushed, release workflow success); `0.21.0` target. |
